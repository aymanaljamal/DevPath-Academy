...
if(mode==='content'||mode==='all'){
  ...
  const platform=await readFile(join(root,'src','scripts','devpath-platform.js'),'utf8');for(const marker of ['LEARNING OBJECTIVES','RUNNABLE POSTGRESQL EXAMPLE','Expected output','COMMON MISTAKES','BEST PRACTICES','REVIEW QUESTIONS','LESSON SUMMARY'])check(platform.includes(marker),`Shared lesson renderer missing ${marker}`);
}
