...
function noteDialog(lesson, note) {
  ...
  return `<dialog id="lessonNoteDialog" class="devpath-dialog notes-studio"><form method="dialog"><button class="dialog-x" value="cancel" aria-label="Close">×</button><span class="eyebrow">${t('personalNotes').toUpperCase()}</span><h2>${esc(lesson.title)}</h2><div class="note-stats"><div><strong>${stats.rows.length}</strong><span>${t('notes')}</span></div><div><strong>${stats.paths}</strong><span>${t('courses')}</span></div><div><strong>${stats
      .words}</strong><span>Words</span></div></div><div class="notes-workspace"><div class="note-editor"><label for="lessonNoteSubject">${t('noteSubject')}</label><input id="lessonNoteSubject" maxlength="160" value="${esc(record.subject || lesson.title)}"><label for="lessonNoteText">${t('noteContent')}</label><textarea id="lessonNoteText" maxlength="12000" placeholder="Capture an insight, question, or code idea…">${esc(record.body)}</textarea></div><aside class="notes-index"><strong>${t('allNotes')}</strong><div>${stats.rows.length ?
      stats.rows
        .map(row => {
          const item = entries.get(row.id);
          return item ?
            `<a href="${item.href}"><b>${esc(row.subject ||
              item.title)}</b><small>${esc(item.courseTitle)} · ${esc(item.moduleTitle)}</small></a>` :
            ''
        })
        .join('') :
      `<p>${t('noNotes')}</p>`}</div></aside></div><div class="note-dialog-actions"><button id="exportAcademyNotesPdf" type="button">${icon('print')}<span>${t('exportPdf')}</span></button><button id="exportAcademyNotesJson" type="button">${icon('projects')}<span>${t('exportJson')}</span></button><button value="cancel">${t('cancel')}</button><button id="saveLessonNote" value="default">${t('saveNote')}</button></div></form></dialog>`;
}
...
function exportAcademyNotesPdf() {
  ...
  popup.opener = null;
  popup.document.write(`<!doctype html><html lang="${state.language}" dir="${state.language === 'ar' ?
      'rtl' :
      'ltr'}"><head><meta charset="utf-8"><title>DevPath Academy Notes</title><style>@page{size:A4;margin:17mm}body{font:11pt/1.6 Arial;color:#201b1b}.cover{min-height:245mm;display:grid;align-content:center;border-left:7px solid #b91c1c;padding:24mm}.cover h1{font-size:30pt}.note{break-inside:avoid;border:1px solid #ddd;border-radius:9px;padding:7mm;margin-bottom:7mm}.note h2{color:#b91c1c}.meta{color:#666;border-bottom:1px solid #eee;padding-bottom:3mm}.body{white-space:pre-wrap}</style></head><body><section class="cover"><small>DEVPATH ACADEMY</small><h1>${t('personalNotes')}</h1><p>${stats.rows.length} ${t('notes')} · ${stats.words} words</p></section>${stats.rows
      .map((row, index) => {
        const item = entries.get(row.id);
        return `<article class="note"><h2>${index + 1}. ${esc(row.subject || item?.title ||
          row.id)}</h2><p class="meta">${esc(item?.courseTitle || '')} · ${esc(item?.moduleTitle ||
...
function footer() {
  return `<footer class="devpath-footer"><a class="devpath-logo" href="#/">${icon(
    'brand',
...
function renderSearchDialog(mode = 'search') {
  ...
  dialog.innerHTML =
    `<form method="dialog"><button class="dialog-x" value="cancel">×</button><h2>${title}</h2>${mode === 'search' ?
  ...
  const results = $('#globalResults', dialog), render = q => {
    ...
    results.innerHTML = match.length ?
      match
        .map(
          l => `<a href="${l.href}" style="--course:${l.color}"><i></i><span><strong>${esc(l.title)}</strong><small>${esc(l.courseTitle)} · ${esc(l.moduleTitle)}${mode === 'notes' ?
              `<br>${esc((state.notes[l.id]?.body || state.notes[l.id] ||
                '').slice(0, 100))}` :
              ''}</small></span><em>${l.type || 'lesson'}</em></a>`)
        .join('') :
      `<div class="empty-state">No ${mode === 'search' ? 'matching courses, modules, or lessons' :
        mode} yet.</div>`;
  };
  render('');
...
function openCourseToolPanel(mode, course) {
  ...
  dialog
    .innerHTML = `<form method="dialog"><button class="dialog-x" value="cancel" aria-label="Close">×</button><span class="eyebrow">${isReview ? 'SMART REVIEW' : 'LEARNING DASHBOARD'}</span><h2>${esc(course.title)}</h2><div class="tool-panel-stats"><div><strong>${progress.percent}%</strong><span>Progress</span></div><div><strong>${done.size}</strong><span>Completed</span></div><div><strong>${bookmarks.length}</strong><span>Bookmarks</span></div><div><strong>${notes
      .length}</strong><span>Notes</span></div></div><div class="tool-panel-list"><strong>${isReview ? 'Continue with these lessons' : 'Course activity'}</strong>${(isReview ? remaining.slice(0, 8) :
      lessons
        .filter(
        ...
        .reverse())
      .map(
        item => `<a href="${routeHref(course.id, item.slug)}"><span>${esc(item.title)}</span><small>${done.has(item.id) ? 'Completed' :
...
function bind() {
  ...
  $('#themeButton')?.addEventListener('click', e => {
    ...
    e.currentTarget.innerHTML =
      `${icon(state.theme === 'dark' ? 'sun' : 'moon')}<span>${state.theme === 'dark' ? 'Light' : 'Night'}</span>`;
  });
  $('#globalSearchButton')
  ...
  if (lesson) {
    ...
    if (themeTool && !$('[data-toolbar-dashboard]'))
      themeTool.insertAdjacentHTML(
        'beforebegin',
        `<button data-toolbar-dashboard title="Course dashboard">${icon(
          'projects')}<span>Dashboard</span></button><button data-toolbar-review title="Smart review">${icon('search')}<span>Review</span></button>`);
    const syncTools = () => {
  ...
  if (lesson) {
    ...
    if (themeTool && !$('[data-toolbar-language]'))
      themeTool.insertAdjacentHTML(
        'beforebegin',
        `<button data-toolbar-language title="Arabic / English">${icon('projects')}<span>${state.language === 'ar' ? 'EN' : 'AR'}</span></button>`);
    const labels = [
