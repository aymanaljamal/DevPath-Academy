function sourcesPage() {
  ...
  if (catalogGroups.length)
    return `${
        header()}<main class="devpath-main info-page sources-page"><a class="back-link" href="#/">← Home</a><span class="eyebrow">VERIFIED PRIMARY DOCUMENTATION</span><h1>Official learning sources</h1><p class="info-lead">${
        Object.values(officialCatalog)
            .reduce(
                (sum, list) => sum + list.length,
                0)} verified official and specification references, grouped by learning path. SQL and optimization lessons select the most relevant PostgreSQL source at lesson level.</p><section class="sources-hero"><article><small>WHY THIS PAGE EXISTS</small><h2>Trust the primary source first.</h2><p>Each course points to the official docs that define the APIs, syntax, and platform rules we teach. Next.js now gets a dedicated source set so the path feels current and anchored in the right references.</p></article><article><small>QUICK GUIDE</small><ul><li>Use the course card to jump into a path.</li><li>Use the source grid to verify a topic.</li><li>Favor official docs before tutorials or blog posts.</li></ul></article></section><div class="source-grid">${
        catalogGroups
            .map(
                ([title, links, courseId]) => `<section class="source-card${
                    courseId === 'nextjs' ? ' is-featured' :
                                            ''}"><h2>${esc(title)}</h2>${
                    links
                        .map(
                            ([label, url]) => `<a href="${
                                url}" target="_blank" rel="noopener noreferrer"><span>${
                                esc(label)}</span><small>${
                                new URL(url).hostname}</small><b>↗</b></a>`)
                        .join('')}</section>`)
            .join('')}</div></main>${footer()}`;
  return `${
      header()}<main class="devpath-main info-page sources-page"><a class="back-link" href="#/">← Home</a><span class="eyebrow">PRIMARY DOCUMENTATION</span><h1>Official learning sources</h1><p class="info-lead">References are separated by learning path so you can verify every topic against its official documentation.</p><section class="sources-hero"><article><small>WHY THIS PAGE EXISTS</small><h2>Trust the primary source first.</h2><p>Every path should be traceable to its official docs. If a path is missing here, it should not be considered complete.</p></article><article><small>QUICK GUIDE</small><ul><li>Use the course card to jump into a path.</li><li>Use the source grid to verify a topic.</li><li>Favor official docs before tutorials or blog posts.</li></ul></article></section><div class="source-grid">${
      groups
          .map(
              ([title,
                links]) => `<section class="source-card"><h2>${title}</h2>${
                  links
                      .map(
                          ([label, url]) => `<a href="${
                              url}" target="_blank" rel="noopener noreferrer"><span>${
...
function aboutPage() {
  return `${
      header()}<main class="devpath-main info-page"><a class="back-link" href="#/">← Home</a><span class="eyebrow">ABOUT THE ACADEMY</span><h1>Built for developers who keep growing.</h1><p class="info-lead">DevPath Academy is a modular learning platform for React, Java, Spring Boot, projects, and future technologies. Every path shares progress, notes, bookmarks, search, responsive navigation, and a consistent lesson experience.</p><section class="creator-panel"><div class="creator-avatar">AA</div><div><small>CREATOR</small><h2>Ayman Aljamal · أيمن الجمل</h2><p>Designed and crafted as a practical path from fundamentals to professional software development.</p><a href="https://github.com/aymanaljamal" target="_blank" rel="noopener noreferrer">github.com/aymanaljamal ↗</a></div></section></main>${
      footer()}`;
}
function render() {
