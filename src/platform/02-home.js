function improvedHome() {
  const recent =
      Object.entries(state.lastLesson)
          ...
          .map(([courseId, slug]) => {
            ...
            return lesson ?
                `<a href="${routeHref(courseId, slug)}"><strong>${
                    esc(lesson.title)}</strong><small>${esc(course.title)} · ${
                    esc(lesson.moduleTitle)}</small></a>` :
                ''
          })
          .join('');
  ...
  return `${
      header()}<main class="devpath-main"><section class="catalog-hero improved-hero"><div class="hero-copy"><span class="hero-kicker">ONE ACADEMY · MANY DEVELOPMENT PATHS</span><h1>Learn by doing.<br><em>Ship what matters.</em></h1><p>Build a practical developer path across frontend, backend, APIs, data, cloud, SQL, and performance, with React refreshed and Next.js now standing beside it as the modern continuation path.</p><div class="hero-proof" aria-label="Academy overview"><span><strong>${
      catalog.length}</strong> focused paths</span><span><strong>${
      lessonTotal}</strong> practical lessons</span><span><strong>Next.js</strong> featured</span></div><div class="hero-search"><input id="homeSearchInput" placeholder="Search courses, modules, or lessons…" aria-label="Search all topics"><button id="heroSearch">Search</button></div><div class="hero-actions"><a class="primary-cta" href="#plan-builder">Build my learning plan ${
      icon(
          'next')}</a><a class="secondary-cta" href="#courses">Explore all paths</a></div></div><div class="path-explorer" style="--active-course:${
      featured
          .color}"><div class="path-orbit" role="tablist" aria-label="Explore learning paths">${
      catalog
          .map(
              (course, index) => `<button class="path-node${
                  index === 0 ? ' active' : ''}" data-path-course="${
                  course.id}" role="tab" aria-selected="${
                  index ===
                  0}" title="${esc(course.title)}" style="--node-color:${
                  course.color}">${technologyIcon(course)}<span>${
                  esc(course.shortTitle || course.title)}</span></button>`)
          .join(
              '')}</div><article class="path-preview"><span id="pathPreviewLabel">INTERACTIVE PATH</span><div id="pathPreviewIcon" class="path-preview-icon">${
      technologyIcon(featured)}</div><h2 id="pathPreviewTitle">${
      esc(featured.title)}</h2><p id="pathPreviewDescription">${
      esc(featured
              .description)}</p><div class="path-preview-stats"><span><strong id="pathPreviewLessons">${
      featuredProgress
          .total}</strong> lessons</span><span><strong id="pathPreviewProgress">${
      featuredProgress
          .percent}%</strong> complete</span></div><a id="pathPreviewLink" href="${
      routeHref(featured.id)}">Explore this path ${
      icon('next')}</a></article></div></section>${learningPulse()}${
      homePlanBuilder()}${
      fileDistributionStudio()}<section class="home-insights"><article><span class="eyebrow">RECENTLY VISITED</span><div class="recent-list">${
      recent ||
      '<p class="empty-state">Open a lesson and it will appear here.</p>'}</div></article><article><span class="eyebrow">POPULAR SKILLS</span><div class="featured-list"><a href="${
      routeHref('react')}">Modern React architecture</a><a href="${
      routeHref('nextjs')}">Build with Next.js App Router</a><a href="${
      routeHref(
          'database-optimization')}">Read query execution plans</a></div></article><article><span class="eyebrow">LEARNING LOOP</span><div class="compact-roadmap"><b>1</b><span>Learn</span><i>→</i><b>2</b><span>Practice</span><i>→</i><b>3</b><span>Ship</span></div></article></section><section class="catalog-section"><div class="section-heading"><div><span class="eyebrow">LEARNING PATHS</span><h2>Choose where to grow next</h2></div><div class="course-filters" role="group" aria-label="Filter courses"><button class="active" data-filter="all">All</button><button data-filter="frontend">Frontend</button><button data-filter="backend">Backend</button><button data-filter="ai">AI &amp; Data</button><button data-filter="projects">Projects</button><button data-filter="foundations">Foundations</button></div></div><div class="course-grid">${
      catalog.map(courseCard).join('')}</div></section></main>${footer()}`;
}
function fileDistributionStudio() {
  return `<section class="file-studio" aria-labelledby="fileStudioTitle"><div class="section-heading"><div><span class="eyebrow">FILE DISTRIBUTION MAP</span><h2 id="fileStudioTitle">How the academy is organized</h2></div><p>Each course lives in its own file, while shared generation and UI stay in common layers.</p></div><div class="file-grid"><article class="file-stack react-stack"><small>React path</small><strong>Updated separately</strong><p>React keeps its own course file and benefits from the shared rendering engine, so the full 18 chapters stay stable while the presentation stays fresher.</p><div class="file-badges"><span>src/data/courses/react.js</span><span>src/content/chapters/</span><span>src/platform/</span></div></article><article class="file-stack"><small>Course files</small><strong>One path per file</strong><p>Each track is a standalone course definition, which makes adding or editing a path low-risk and easy to review.</p><div class="file-badges"><span>nextjs.js</span><span>spring-boot.js</span><span>python-ai.js</span></div></article><article class="file-stack"><small>Shared layers</small><strong>Reusable logic</strong><p>Common lesson generation, catalog rendering, and UI interactions live in shared scripts so every path inherits the same platform behavior.</p><div class="file-badges"><span>zz-lesson-content.js</span><span>02-home.js</span><span>devpath-platform.css</span></div></article></div></section>`;
}
function homePlanBuilder() {
  return `<section class="plan-builder" id="plan-builder" aria-labelledby="planBuilderTitle"><div class="plan-builder-intro"><span class="eyebrow">PERSONAL ROADMAP BUILDER</span><h2 id="planBuilderTitle">Turn your goal into a clear weekly plan.</h2><p>Choose what you want to build, tell us your starting point, and set a realistic weekly pace. Your roadmap updates instantly.</p><div class="plan-benefits"><span>${
      icon('check')} Ordered foundations</span><span>${
      icon('check')} Realistic timeline</span><span>${
      icon(
...
function setupPlanBuilder() {
  ...
  const render = () => {
    ...
    output.innerHTML =
        `<div class="plan-output-head"><div><small>YOUR RECOMMENDED TRACK</small><h3>${
            labels[goal]}</h3></div><div><strong>${
            weeks}</strong><span>estimated weeks</span></div></div><div class="plan-route">${
            selected
                .map((id, index) => {
                  const course = courses[id];
                  return `<a href="${routeHref(id)}" style="--plan-color:${
                      course.color}"><b>${
                      String(index + 1).padStart(
                          2, '0')}</b><i>${technologyIcon(course)}</i><span>${
                      esc(course.shortTitle || course.title)}</span><small>${
                      courseProgress(course).total} lessons</small></a>`
                })
                .join('')}</div><p>At <strong>${
            hours} hours/week</strong>, focus on one path at a time and build a small project after every two stages.</p>`;
  };
...
function learningPulse() {
  ...
  return `<section class="learning-pulse" aria-labelledby="learningPulseTitle"><div><span class="eyebrow">YOUR LEARNING PULSE</span><h2 id="learningPulseTitle">Small steps, real momentum.</h2><p>${
      lesson ?
          `Continue <strong>${esc(lesson.title)}</strong> in ${
              esc(course.shortTitle || course.title)}.` :
          'Open a lesson and your personal learning pulse will start here.'}</p><div class="pulse-actions"><a class="primary-cta" href="${
      continueHref}">${lesson ? 'Continue learning' : 'Start a path'} ${
      icon(
          'next')}</a><button id="smartReviewHome" class="secondary-cta" type="button">Pick my next lesson</button></div></div><div class="pulse-stats"><article><strong>${
      learningStreak()}</strong><span>day streak</span></article><article><strong>${
      today}</strong><span>done today</span></article><article><strong>${
      done}</strong><span>lessons complete</span></article><article><strong>${
      overall()}%</strong><span>academy progress</span></article></div></section>`;
}
function homeInteractions() {
  return `<section class="home-playground" aria-labelledby="playgroundTitle"><div class="section-heading"><div><span class="eyebrow">INTERACTIVE LAB</span><h2 id="playgroundTitle">Make your next move</h2></div><p>Three quick tools that turn browsing into a learning decision.</p></div><div class="playground-grid"><article class="path-finder"><div class="playground-icon">${
      icon(
          'search')}</div><span>PATH FINDER</span><h3>What do you want to build?</h3><div class="finder-options" role="group" aria-label="Choose a learning goal"><button data-find-path="react">Interactive interfaces</button><button data-find-path="spring-boot">Production APIs</button><button data-find-path="java-essentials">Strong programming foundations</button><button data-find-path="projects">Portfolio projects</button></div><div class="finder-result" id="finderResult" aria-live="polite"><small>Choose a goal</small><strong>Your recommended path appears here.</strong></div></article><article class="challenge-machine"><div class="playground-icon">${
      icon(
          'projects')}</div><span>CHALLENGE DECK</span><h3>Draw a practical challenge</h3><div class="challenge-card" id="challengeCard"><small>CHALLENGE 01</small><strong>Build a lesson progress component that persists after refresh.</strong><p>Focus: state, storage, and accessible controls.</p></div><button class="playground-action" id="nextChallenge" type="button">${
      icon(
          'next')}<span>Draw another</span></button></article><article class="study-planner"><div class="playground-icon">${
      icon(
...
function setupHomeInteractions() {
  ...
  $$('[data-find-path]')
      .forEach(button => button.addEventListener('click', () => {
        ...
        result.innerHTML = `<small>RECOMMENDED PATH</small><strong>${
            esc(course.title)}</strong><a href="${
            routeHref(course.id)}">Open course ${icon('next')}</a>`;
      }));
  const challenges = [
    [
  ...
  $('#nextChallenge')?.addEventListener('click', () => {
    ...
    card.innerHTML = `<small>CHALLENGE ${
        String(challengeIndex + 1).padStart(2, '0')}</small><strong>${
        title}</strong><p>Focus: ${focus}</p>`;
  });
  const updatePlanner = () => {
...
function setupJourneyStudio() {
    ...
    $('#goalPath').innerHTML =
        maps[goal]
            .map(id => {
              ...
              return `<a href="${routeHref(id)}" style="--path-color:${
                  course.color}"><i>${technologyIcon(course)}</i><span>${
                  esc(course.shortTitle)}</span><small>${
                  p.percent}%</small></a>`
            })
            .join('<b>→</b>');
  ...
  $$('[data-constellation-course]')
      .forEach(button => button.addEventListener('click', () => {
        ...
        detail.innerHTML = `<small>${
            p.done ? `${p.done} LESSONS COMPLETED` :
                     'READY TO BEGIN'}</small><strong>${
            esc(course.title)}</strong><p>${
            esc(course.description)}</p><a href="${routeHref(course.id)}">${
            p.done ? 'Continue' : 'Explore'} path ${icon('next')}</a>`;
      }));
}
  ...
  const p = courseProgress(course),
      ...
      (course.id === 'react'        ? 'frontend' :
           course.id === 'projects' ? 'projects' :
                                      'backend');
  return `<article class="course-card" data-type="${type}" style="--course:${
      course.color}"><div class="course-card-top"><span class="tech-icon">${
      technologyIcon(course)}</span><span class="course-level">${
      course.level}</span></div><h3>${esc(course.title)}</h3><p>${
      esc(course.description)}</p><div class="course-meta"><span>${
      p.total} lessons</span><span>${
      course
          .duration}</span></div><div class="card-progress"><div><span>Progress</span><strong>${
      p.percent}%</strong></div><progress max="100" value="${p.percent}">${
      p.percent}%</progress></div><a href="${routeHref(course.id)}">${
      p.done ?
          'Continue course' :
