function setupFrameworkLabs() {
  ...
  const setSpringCase = key => {
    ...
    $('#springPipeline').innerHTML =
      item.steps
        .map(
          ([name, detail], index) => `<div class="${index === item.steps.length - 1 ?
              'final' :
              ''}"><b>${index + 1}</b><span><strong>${name}</strong><small>${detail}</small></span></div>`)
        .join('');
  ...
  const renderPostman = () => {
    ...
    $('#postmanTestPreview').textContent = `pm.test("returns ${method === 'POST' ?
        '201' :
        '2xx'}", () => {\n  pm.expect(pm.response.code).to.be.${method === 'POST' ?
        'oneOf([200, 201])' :
...
function setupDatabaseVisuals() {
  ...
  const renderQueryBuilder = () => {
    const filter = $('#queryFilter')?.value || 'all',
      ...
      sql =
        `SELECT ${select}\nFROM orders${where ? `\nWHERE ${where}` : ''}${group !== 'none' ? `\nGROUP BY ${group}` : ''}\nORDER BY ${group === 'none' ? order : 'revenue DESC'};`;
    ...
    if (group !== 'none') {
      ...
      $('#queryBuilderResult').innerHTML =
        `<div><b>${group}</b><b>orders</b><b>revenue</b></div>${rows.map(
          row => `<div><span>${esc(row.key)}</span><span>${row.count}</span><span>$${row.total}</span></div>`)
          .join('')}`;
    } else {
      rows.sort(
      ...
      $('#queryBuilderResult').innerHTML =
        `<div><b>customer</b><b>status</b><b>total</b></div>${rows.map(
          row => `<div><span>${esc(row.customer)}</span><span>${row.status}</span><span>$${row.total}</span></div>`)
          .join('')}`;
    }
...
function pythonToolPage(tool) {
  ...
  let body = '';
  if (tool === 'roadmap')
    body = `<div class="roadmap-grid">${course.modules
        .map(
          (module, index) =>
            `<a href="#module-${module.id}" data-course-return><b>${String(index + 1).padStart(
              2,
              '0')}</b><span>${esc(module.title)}</span><small>${module.lessons.length} lessons · ${index < 4 ? 'FOUNDATION' :
  ...
  if (tool === 'models')
    body = `<div class="resource-controls"><input id="resourceSearch" type="search" placeholder="Search models…"><select id="resourceFilter"><option value="all">All families</option><option>Regression</option><option>Classification</option><option>Clustering</option><option>Deep Learning</option></select></div><div class="resource-grid" id="resourceGrid">${pythonModels
            ...
            index < 10 ? 'Classification' :
              index < 13 ? 'Clustering' :
                index < 14 ? 'Time Series' :
                  'Deep Learning';
          return `<article data-family="${family}" data-search="${name.toLowerCase()}"><span>${family}</span><h2>${name}</h2><p>Learn mechanics, assumptions, preprocessing, hyperparameters, cost, explainability, evaluation, and evidence-based comparisons.</p><a href="${routeHref(
            'python-ai',
            index < 8 ?
        ...
        .join('')}</div>`;
  if (tool === 'projects')
    body = `<div class="resource-grid">${course.capstones
        .map(
          (name, index) => `<article><span>CAPSTONE ${String(index + 1).padStart(2, '0')}</span><h2>${name}</h2><p>Business goal, dataset card, EDA, cleaning, features, baseline, multiple candidates, tuning, final evaluation, explainability, deployment, README, repository structure, and portfolio copy.</p><a href="${routeHref(
            'python-ai',
  ...
  if (tool === 'datasets')
    body =
      `<p class="tool-intro">Small open or generated datasets only. Every generated dataset uses seed 42 and can be recreated without downloading binaries.</p><div class="dataset-table"><div><b>Dataset</b><b>Task</b><b>Description</b></div>${pythonDatasets
        .map(
          row => `<div>${row.map(cell => `<span>${esc(cell)}</span>`)
              .join('')}</div>`)
        .join('')}</div>`;
  if (tool === 'cheatsheets')
    body = `<div class="resource-grid">${[['Python Core', 'collections · functions · files · exceptions'],
      ...
      ['Responsible AI', 'privacy · fairness · safety · documentation']]
        .map(
          ([name,
            text]) => `<article><span>QUICK REFERENCE</span><h2>${name}</h2><p>${text}</p><button class="print-sheet" type="button">Print / Save PDF</button></article>`)
        .join('')}</div>`;
  if (tool === 'glossary')
    body =
      `<div class="resource-controls"><input id="resourceSearch" type="search" placeholder="Search English or Arabic terms…"></div><dl class="glossary" id="resourceGrid">${[['Feature', 'سمة', 'An input variable used by a model.'],
      [
        'Label', 'تسمية',
      ...
      [
        ...
        .map(
          ([en, ar, definition]) => `<div data-search="${en.toLowerCase()} ${ar}"><dt>${en} <span>${ar}</span></dt><dd>${definition}</dd></div>`)
        .join('')}</dl>`;
  if (tool === 'lab') body = modelComparisonLab();
  return `${header()}<main class="devpath-main python-tool-page" style="--course:${course.color}"><a class="back-link" href="${routeHref(
    'python-ai')}">← Python &amp; AI</a><span class="eyebrow">PYTHON &amp; AI TOOLKIT</span><h1>${title}</h1>${body}</main>${footer()}`;
}
...
function setupPythonTools() {
      ...
      if (saved)
        saved.innerHTML = rows.length ?
          rows.slice()
            ...
            .map(
              row =>
                `<article><strong>${esc(row.best)}</strong><span>${esc(row.problem)} · ${row.models.length} models</span><small>${new Date(row.date)
  ...
  form?.addEventListener('submit', event => {
    ...
    $('#labResults')
      .innerHTML = `<div class="lab-verdict"><strong>Best candidate: ${esc(best.name)}</strong><p>Selected by the primary ${problem === 'classification' ?
        'F1' :
        'R²'} metric on the same seeded split. Confirm with cross-validation, error plots, and business cost before deployment.</p></div><div class="lab-table"><div>${headers.map(h => `<b>${h}</b>`).join('')}</div>${scored
        .map(
          row => `<div>${Object.values(row)
              .map(
                (v, i) => `<span>${i ? typeof v === 'number' && v < 1 ?
                    v.toFixed(3) :
                    v :
                    v}</span>`)
              .join('')}</div>`)
        .join(
          '')}</div><div class="lab-chart" aria-label="Metric comparison">${scored
        .map(
          row => `<div><span>${esc(row.name)}</span><i style="--score:${Math.round((row.f1 || row.r2) * 100)}%"></i><b>${Math.round((row.f1 || row.r2) * 100)}%</b></div>`)
        .join('')}</div><div class="lab-visual"><h3>${problem === 'classification' ? 'Confusion matrix & ROC' :
        'Residual plot'}</h3><p>${problem === 'classification' ?
