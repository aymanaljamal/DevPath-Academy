function coursePage(course) {
  ...
  const pythonTools = course.id === 'python-ai' ?
    `<nav class="python-tool-nav" aria-label="Python and AI learning tools">${[['roadmap', 'Roadmap'], ['models', 'Models Library'],
    ...
    ['lab', 'Model Comparison Lab']]
      ...
      .join('')}</nav>` :
    '';
  return `${header()}<main class="devpath-main course-page premium-course-page" style="--course:${course
      .color}"><a class="back-link" href="#/">← All learning paths</a><section class="course-hero premium-hero"><div class="course-hero-copy"><span class="eyebrow">${esc(course.level)}</span><h1>${esc(state.language === 'ar' && course.titleAr ? course.titleAr :
        course.title)}</h1><p>${esc(state.language === 'ar' && course.descriptionAr ?
          course.descriptionAr :
          course.description)}</p><div class="course-stats"><span><strong>${course.modules.length}</strong> chapters</span><span><strong>${lessons.length}</strong> lessons</span><span><strong>${p.percent}%</strong> complete</span></div><div class="course-actions"><a class="primary-cta" href="#module-${course.modules[0]
      .id}">Start Learning</a><a class="secondary-cta" href="#/sources">Explore Curriculum</a></div></div><div class="course-hero-panel"><div class="hero-panel-chip">${course.modules.length} Chapter Path</div><div class="hero-panel-title">${esc(course.shortTitle || course.title)}</div><p>${esc(course.id === 'file-extensions' ?
        'Learn to identify source, configuration, generated, secret, and deployment files by purpose rather than memorizing suffixes.' :
        'A focused path with reusable structure, stable progress, and clear chapter hierarchy.')}</p><div class="hero-panel-progress"><span><strong>${p.percent}%</strong> Progress</span><progress max="100" value="${p.percent}"></progress></div></div></section>${course.id === 'file-extensions' ? fileExtensionsLanding() :
      course.id === 'spring-boot' ? springPipelineLab() :
        course.id === 'postman' ? postmanRequestLab() :
          ''}${pythonTools}<div class="course-layout premium-layout"><aside class="module-nav premium-module-nav"><strong>Course</strong><div class="module-nav-title">${esc(course.title)}</div><label class="module-search"><span>${icon(
            'search')}</span><input type="search" placeholder="Search chapters"></label><div class="module-nav-progress"><span>Progress</span><strong>${p.percent}%</strong><progress max="100" value="${p.percent}"></progress></div>${course.modules
      .map(
        (m, i) => `<a href="#module-${m.id}"><span>${String(i + 1).padStart(2, '0')}</span>${esc(state.language === 'ar' && m.titleAr ? m.titleAr :
          m.title)}</a>`)
      .join('')}</aside><section class="module-list">${course.modules.map((module, i) => moduleCard(course, module, i))
      .join(
...
function reactCoursePage(course) {
  ...
  const experienceCard = experience ?
    `<article class="module-card project-experience-card" id="react-project-structure"><div class="module-heading"><div><span>INTERACTIVE EXPERIENCE</span><h2>${esc(experience
    [1])}</h2></div><small>Explorer · flow · quiz</small></div><ol><li><a href="${routeHref(course.id, experience[0])}"><span class="lesson-status">${(state.completed.react || []).includes(`react:${experience[0]}`) ?
      '✓' :
      '↗'}</span><span><small>Visual architecture lab</small><strong>Open the React project explorer</strong></span><b>→</b></a></li></ol></article>` :
    '';
  return `${header()}<main class="devpath-main course-page react-original premium-course-page" style="--course:${course
      .color}"><a class="back-link" href="#/">← All learning paths</a><section class="course-hero premium-hero"><div class="course-hero-copy"><span class="eyebrow">YOUR ORIGINAL COURSE · FULL CONTENT</span><h1>${esc(course.title)}</h1><p>${esc(course
        .description)}</p><div class="course-stats"><span><strong>18</strong> chapters</span><span><strong>416</strong> sections</span><span><strong>${p.percent}%</strong> complete</span></div><div class="course-actions"><a class="primary-cta react-launch" href="react.html#chapter-1">Continue original course</a><a class="secondary-cta" href="${routeHref(
          'react',
          'project-structure')}">Explore project structure</a></div></div><div class="course-hero-panel"><div class="hero-panel-chip">Preserved Reader + Visual Lab</div><div class="hero-panel-title">React</div><p>The original curriculum stays preserved while a new interactive architecture experience explains how real project files connect.</p><div class="hero-panel-progress"><span><strong>${p.percent}%</strong> Progress</span><progress max="100" value="${p.percent}"></progress></div></div></section>${reactRenderLab()}<div class="course-layout premium-layout"><aside class="module-nav premium-module-nav"><strong>Original course outline</strong><div class="module-nav-title">React Developer Course</div><a href="#react-project-structure"><span>00</span>Project Structure Lab</a>${course.modules
      .map(
        (module, index) => `<a href="#react-module-${module.id}"><span>${String(index + 1).padStart(
          2, '0')}</span>${esc(module.title)}</a>`)
      .join('')}</aside><section class="module-list">${experienceCard}${course.modules
      .map(
        (module,
          moduleIndex) => `<article class="module-card" id="react-module-${module.id}"><div class="module-heading"><div><span>PART ${String(moduleIndex + 1).padStart(2, '0')}</span><h2>${esc(module.title)}</h2></div><small>${module.lessons.length} original chapters</small></div><ol>${module.lessons
            .map(([slug, title]) => {
              number += 1;
              const completed = completedChapters.has(slug);
              return `<li class="${completed ? 'done' : ''}"><a href="react.html#${slug}"><span class="lesson-status">${completed ?
                  '✓' :
                  number}</span><span><small>Original chapter ${number}</small><strong>${esc(title)}</strong></span><b>→</b></a></li>`;
            })
            .join('')}</ol></article>`)
      .join(
        '')}</section><aside class="lesson-toc" aria-label="On this page"><strong>On this page</strong><a href="#react-project-structure">Project lab</a><a href="#react-render-lab">Render lab</a><a href="#react-module-react-foundations">Foundations</a><a href="#react-module-react-core">Core</a><a href="#react-module-react-enterprise">Enterprise</a><a href="#react-module-react-production">Production</a></aside></div></main>${footer()}`;
}
...
function moduleCard(course, module, index) {
  ...
  return `<article class="module-card" id="module-${module.id}"><div class="module-heading"><div><span>${course.id === 'python-ai' ?
      'STAGE' :
      'MODULE'} ${String(index + 1).padStart(2, '0')}</span><h2>${esc(state.language === 'ar' && module.titleAr ?
        module.titleAr :
        module.title)}</h2></div><small>${module.lessons.filter(([slug]) => done.has(`${course.id}:${slug}`))
      .length}/${module.lessons.length} complete</small></div><ol>${module.lessons
      .map(
        ([slug, title, meta = {}], i) => `<li class="${done.has(`${course.id}:${slug}`) ? 'done' : ''}"><a href="${routeHref(course.id, slug)}"><span class="lesson-status">${done.has(`${course.id}:${slug}`) ?
            '✓' :
            i + 1}</span><span><small>${meta.kind === 'assessment' ?
            'Assessment' :
            meta.kind === 'review' ?
              'Review' :
              `Lesson ${index + 1}.${i + 1}`}</small><strong>${esc(state.language === 'ar' && meta.titleAr ?
                meta.titleAr :
...
function reactLanding() {
  ...
  return `${header()}<main class="devpath-main course-page" style="--course:#169fca"><a class="back-link" href="#/">← All learning paths</a><section class="course-hero"><span class="tech-icon large">R</span><div><span class="eyebrow">ORIGINAL COURSE · FULLY PRESERVED</span><h1>React Developer Course</h1><p>${reactCourse
...
function lessonPage(course, lesson) {
  ...
  const experienceSidebar = course.experiences?.length ?
    `<details open><summary>Interactive Experiences</summary>${course.experiences
      .map(([slug, itemTitle]) => `<a class="${slug === lesson.slug ? 'active' : ''}" href="${routeHref(course.id, slug)}">${esc(itemTitle)}</a>`)
      .join('')}</details>` :
    '';
  return `${header()}<main class="devpath-main lesson-page premium-lesson-page" style="--course:${course
      .color}"><aside class="lesson-sidebar premium-lesson-sidebar"><a class="back-link" href="${routeHref(course.id)}">← ${esc(course
        .shortTitle)}</a><strong>Course contents</strong><div class="lesson-sidebar-search"><span>${icon(
          'search')}</span><input type="search" placeholder="Search chapters"></div>${course.modules
      .map(
        m => `<details ${m.id === lesson.moduleId ? 'open' : ''}><summary>${esc(state.language === 'ar' && m.titleAr ?
          m.titleAr :
          m.title)}</summary>${m.lessons
            .map(
              ([slug, itemTitle, meta = {}]) => `<a class="${slug === lesson.slug ?
                  'active' :
                  ''}" href="${course.id === 'react' ?
                  `react.html#${slug}` :
                  routeHref(course.id, slug)}">${esc(state.language === 'ar' && meta.titleAr ?
                    meta.titleAr :
                    itemTitle)}</a>`)
            .join('')}</details>`)
      .join(
        '')}${experienceSidebar}</aside><article class="lesson-content premium-lesson-content"><div class="lesson-breadcrumb">${esc(state.language === 'ar' && course.titleAr ?
          course.titleAr :
          course.title)} <span>/</span> ${esc(moduleTitle)}</div><div class="lesson-title-row premium-lesson-header"><div><span class="eyebrow">LESSON ${index + 1} OF ${lessons.length}</span><h1>${esc(title)}</h1>${lesson.duration ?
      `<div class="lesson-meta"><span>${esc(lesson.duration)}</span><span>${esc(lesson.difficulty)}</span><span>Prerequisite: ${esc(lesson.prerequisites)}</span></div>` :
      ''}</div><div class="lesson-actions"><button id="lessonBookmark" class="${bookmarked ? 'is-active' : ''}" aria-pressed="${bookmarked}">${icon('bookmark')}<span>Bookmark</span></button><button id="lessonNote">${icon('notes')}<span>${note ?
      'Edit note' :
      'Add note'}</span></button></div></div><div class="lesson-intro-strip"><span>${course.id === 'react' ? 'React core concepts' :
      'Premium lesson experience'}</span><strong>${course.id === 'react' ?
      'Build with confidence and clarity.' :
      'Learn the concept, prove it, then apply it.'}</strong></div>${lessonSections(
        course, lesson,
        example)}<div class="practice-card"><span>PRACTICE</span><h2>Try it yourself</h2><p>Create a small example for <strong>${esc(title)}</strong>. Explain why each important line exists, run it, and compare the result with your expectation.</p><label><input id="lessonComplete" type="checkbox" ${done ?
      'checked' :
      ''}> I completed this lesson</label></div><nav class="lesson-pager">${prev ? `<a href="${routeHref(
        course.id, prev.slug)}"><small>← Previous</small><strong>${esc(prev.title)}</strong></a>` :
      '<span></span>'}${next ?
      `<a class="next" href="${routeHref(course.id, next.slug)}"><small>Next →</small><strong>${esc(next.title)}</strong></a>` :
      `<a class="next" href="${routeHref(
        course
          .id)}"><small>Course complete</small><strong>Review course outline →</strong></a>`}</nav></article><aside class="lesson-toc premium-lesson-toc" aria-label="On this page"><strong>On this page</strong><a href="#lesson-objectives">Objectives</a><a href="#lesson-concepts">Concepts</a><a href="#lesson-example">Example</a><a href="#lesson-real-world">Real world</a><a href="#lesson-practice">Practice</a><a href="#lesson-questions">Questions</a><a href="#lesson-summary">Summary</a></aside></main>${footer()}${noteDialog(lesson, note)}`;
}
function lessonToolbar(course, lesson) {
  ...
  return `<aside class="academy-toolbar" aria-label="Lesson tools"><div class="toolbar-progress"><span>${esc(course.shortTitle)}</span><strong>${progress.percent}%</strong><progress max="100" value="${progress.percent}"></progress></div><div class="toolbar-actions">${prev ?
      `<a href="${routeHref(
        course.id,
        prev.slug)}" aria-label="Previous lesson" title="Previous lesson">${icon('previous')}</a>` :
      ''}${next ?
      `<a href="${routeHref(
        course.id,
        next.slug)}" aria-label="Next lesson" title="Next lesson">${icon('next')}</a>` :
      ''}<button data-toolbar-bookmark class="${bookmarked ? 'is-active' :
      ''}" aria-pressed="${bookmarked}" title="Bookmark lesson">${icon(
        'bookmark')}<span>Bookmark</span></button><button data-toolbar-note title="Add lesson note">${icon(
          'notes')}<span>Notes</span></button><button data-toolbar-complete class="${done ? 'is-active' : ''}" aria-pressed="${done}" title="Mark lesson complete">${icon('check')}<span>${done ?
      'Completed' :
      'Complete'}</span></button><button data-toolbar-print title="Print or save lesson as PDF">${icon(
        'print')}<span>PDF</span></button><button data-toolbar-theme title="Toggle night theme">${icon(
          state.theme === 'dark' ?
            'sun' :
  ...
  if (course.id === 'python-ai') {
    ...
    const projectBlock = isProject ?
      `<section class="lesson-section project-brief"><span class="section-label">PORTFOLIO PROJECT</span><h2>From business goal to deployment</h2><ol><li>Define the problem, user, business metric, constraints, and dataset.</li><li>Explore, clean, split, and engineer features without leakage.</li><li>Build a baseline and at least three candidate models.</li><li>Cross-validate, tune finalists, evaluate once on the test set, and explain results.</li><li>Document risks, deployment guidance, monitoring, and reproducibility.</li></ol><pre><code class="language-text">project/\n├── README.md\n├── data/README.md\n├── notebooks/01_eda.ipynb\n├── src/{data,features,train,evaluate}.py\n├── tests/\n├── models/.gitkeep\n└── requirements.txt</code></pre>${/ Disease /.test(lesson.title) ?
        ...
        ''}</section>` :
      '';
    return `<section id="lesson-objectives" class="lesson-objectives"><span class="section-label">LEARNING OBJECTIVES</span><ul><li>Explain ${esc(lesson
      .title)} in clear English and Arabic technical language.</li><li>Implement a reproducible example and interpret its output.</li><li>Recognize trade-offs, failure modes, and appropriate alternatives.</li></ul></section><section id="lesson-concepts" class="explanation-grid"><div><span>INTUITIVE VIEW</span><p>${esc(guide.simple)}</p></div><div><span>STEP BY STEP</span><p>${esc(guide
        .technical)}</p></div><div><span>PROFESSIONAL PRACTICE</span><p>${esc(guide.professional)}</p></div></section>${modelBlock}<section id="lesson-example" class="lesson-section"><span class="section-label">RUNNABLE EXAMPLE</span><h2>Build the smallest reproducible experiment</h2><p>Use Python ${esc(course
          .pythonVersion)} and a fixed seed. Install the lesson libraries in a virtual environment: <code>python -m venv .venv</code>, then <code>python -m pip install numpy pandas scikit-learn matplotlib</code>. PyTorch stages use <code>torch</code>.</p><pre><code class="language-${example.language}">${esc(example
            .code)}</code></pre><div class="expected-output"><strong>Expected output</strong><pre><code class="language-text">${esc(example.output)}</code></pre></div></section>${projectBlock}<section id="lesson-practice" class="practice-card exercise-levels"><span>TRY IT YOURSELF</span><h2>Practice at three levels</h2><p><strong>Easy:</strong> reproduce the example and annotate every transformation.</p><p><strong>Medium:</strong> change the data or hyperparameters and explain the result.</p><p><strong>Challenge:</strong> compare an alternative, measure it fairly, and defend your selection.</p></section><section class="lesson-section split"><div><span class="section-label warning">COMMON MISTAKES</span><ul><li>Fitting preprocessing before the data split.</li><li>Reporting one metric or a single lucky split.</li><li>Confusing correlation, prediction, and causation.</li></ul></div><div id="lesson-questions"><span class="section-label good">SHORT QUIZ</span><ol><li>What assumption matters most here?</li><li>Which metric matches the real error cost?</li><li>What evidence would make you reject this approach?</li></ol></div></section><section id="lesson-summary" class="takeaways"><span>LESSON SUMMARY</span><ul><li>Start with the problem and a transparent baseline.</li><li>Keep experiments reproducible and leakage-free.</li><li>Compare alternatives using consistent data and metrics.</li></ul></section>`;
  }
  return `<section id="lesson-objectives" class="explanation-grid"><div><span>SIMPLE EXPLANATION</span><p>${esc(guide.simple)}</p></div><div><span>TECHNICAL EXPLANATION</span><p>${esc(guide
    .technical)}</p></div><div><span>PROFESSIONAL EXPLANATION</span><p>${esc(guide
      .professional)}</p></div></section><section id="lesson-concepts" class="lesson-section"><span class="section-label">SYNTAX &amp; ANNOTATIONS</span><h2>Recognize the working shape</h2><p>Read the declaration, inputs, return type, lifecycle, and annotations from the outside inward.</p><pre><code class="language-${example.language}">${esc(example
        .code)}</code></pre><div class="expected-output"><strong>Expected output</strong><pre><code class="language-text">${esc(example
          .output)}</code></pre></div></section><section class="lesson-section"><span class="section-label">REAL-WORLD USE CASE</span><h2>Use it in the Academy system</h2><p>Apply ${esc(lesson
            .title)} where courses, lessons, learners, and progress must remain correct and maintainable as traffic and requirements grow.</p></section><section id="lesson-practice" class="lesson-section split"><div><span class="section-label warning">COMMON MISTAKES</span><h2>Watch out for</h2><ul><li>Using the API without understanding ownership or lifecycle.</li><li>Mixing domain, transport, and infrastructure responsibilities.</li><li>Ignoring failure paths, resource cleanup, and boundary cases.</li></ul></div><div><span class="section-label good">BEST PRACTICES</span><h2>Use it well</h2><ul><li>Prefer small, intention-revealing units.</li><li>Validate inputs and preserve clear boundaries.</li><li>Test important behavior and observable failures.</li></ul></div></section><section class="practice-card"><span>PRACTICE CHALLENGE</span><h2>Apply the concept</h2><p>${esc(guide
              .exercise)}</p></section><section id="lesson-questions" class="lesson-section"><span class="section-label">INTERVIEW QUESTIONS</span><h2>Check your understanding</h2><ol><li>What problem does ${esc(lesson
...
function detailedLessonSections(course, lesson) {
  ...
  const concepts =
    c.concepts
      .map(
        item => `<article><h3>${esc(item.name)}</h3><p>${esc(item.explanation)}</p>${item.syntax ?
            `<pre><code class="language-${esc(item.language || c.examples[0].language)}">${esc(item.syntax)}</code></pre>` :
            ''}</article>`)
      .join('');
  const examples =
    c.examples
      .map(
        (item, index) => `<article class="lesson-example"><h3>${index + 1}. ${esc(item.title)}</h3><p>${esc(item.explanation)}</p><pre><code class="language-${esc(item.language)}">${esc(item.code)}</code></pre>${item.output ?
            `<div class="expected-output"><strong>Expected behavior or output</strong><pre><code class="language-text">${esc(item.output)}</code></pre></div>` :
            ''}</article>`)
      .join('');
  return `<section id="lesson-objectives" class="lesson-objectives"><span class="section-label">LEARNING OBJECTIVES</span><ul>${c.objectives.map(item => `<li>${esc(item)}</li>`)
      .join(
        '')}</ul></section><section id="lesson-concepts" class="explanation-grid"><div><span>SIMPLE EXPLANATION</span><p>${esc(c.simple)}</p></div><div><span>TECHNICAL EXPLANATION</span><p>${esc(c.technical)}</p></div><div><span>WHEN AND WHY</span><p>${esc(c.whenToUse)}</p></div></section><section class="lesson-section concept-catalog"><span class="section-label">TYPES &amp; MAIN CONCEPTS</span><h2>Important variations and rules</h2><div class="model-facts">${concepts}</div></section>${(course.id === 'sql' || course.id === 'database-optimization') ?
      databaseVisual(lesson.title) :
      ''}<section id="lesson-example" class="lesson-section"><span class="section-label">PRACTICAL EXAMPLES</span><h2>Build understanding step by step</h2>${examples}</section><section class="lesson-section project-brief"><span class="section-label">PROFESSIONAL EXAMPLE</span><h2>A realistic application</h2><p>${esc(c.realWorld.context)}</p><p><strong>Implementation:</strong> ${esc(c.realWorld
        .implementation)}</p><p><strong>Why this design:</strong> ${esc(c.realWorld
          .reasoning)}</p></section><section id="lesson-practice" class="lesson-section split"><div><span class="section-label warning">COMMON MISTAKES</span><ul>${c.mistakes.map(item => `<li>${esc(item)}</li>`)
      .join(
        '')}</ul></div><div><span class="section-label good">BEST PRACTICES</span><ul>${c.practices.map(item => `<li>${esc(item)}</li>`)
      .join(
        '')}</ul></div></section><section class="practice-card"><span>PRACTICE CHALLENGE</span><h2>Apply ${esc(lesson.title)}</h2><p>${esc(c.exercise)}</p></section><section id="lesson-questions" class="lesson-section"><span class="section-label">REVIEW QUESTIONS</span><ol>${c.questions.map(item => `<li>${esc(item)}</li>`)
      .join(
        '')}</ol></section><section id="lesson-summary" class="takeaways"><span>LESSON SUMMARY</span><ul>${c.takeaways.map(item => `<li>${esc(item)}</li>`)
      .join(
        '')}</ul></section><section class="lesson-sources"><span class="section-label">AUTHORITATIVE REFERENCES</span>${sources
      .map(
        source => `<a href="${esc(source
          .url)}" target="_blank" rel="noopener noreferrer"><strong>${esc(source.title)}</strong><small>${esc(source.publisher)} · ${esc(source.type)}</small></a>`)
      .join('')}</section>`;
}
...
function databaseVisual(title) {
  if (/JOIN/i.test(title))
    return `<section class="db-visual" data-db-visual="join"><span class="section-label">INTERACTIVE JOIN VISUALIZER</span><h2>See which rows survive each JOIN</h2><div class="visual-controls" role="group" aria-label="Join type">${['INNER', 'LEFT', 'RIGHT', 'FULL', 'CROSS']
        .map(
          type => `<button type="button" data-join="${type}">${type} JOIN</button>`)
        .join(
  ...
  return `<section class="db-visual"><span class="section-label">QUERY EXECUTION FLOW</span><h2>Follow data from intent to result</h2><div class="execution-flow">${['SQL text', 'Parser', 'Rewriter', 'Planner', 'Executor', 'Rows']
...
function databaseLessonSections(course, lesson, example, guide) {
  ...
  return `<section class="lesson-objectives"><span class="section-label">LEARNING OBJECTIVES</span><ul><li>Explain ${esc(lesson
    .title)} and its prerequisites.</li><li>Write and run correct PostgreSQL syntax.</li><li>Interpret the result and connect it to production behavior.</li></ul></section><section class="explanation-grid"><div><span>SIMPLE EXPLANATION</span><p>${esc(guide.simple)}</p></div><div><span>TECHNICAL EXPLANATION</span><p>${esc(guide.technical)}</p></div><div><span>REAL-WORLD USE</span><p>${esc(guide.professional)}</p></div></section>${databaseVisual(
      lesson
        .title)}<section class="lesson-section"><span class="section-label">RUNNABLE POSTGRESQL EXAMPLE</span><h2>Syntax, result, and interpretation</h2><pre><code class="language-sql">${esc(example
          .code)}</code></pre><div class="expected-output"><strong>Expected output</strong><pre><code>${esc(example
            .output)}</code></pre></div><p>The output confirms whether PostgreSQL accepted the statement or which plan node performed the work. Read estimates separately from actual measurements.</p></section><section class="practice-card"><span>INTERACTIVE EXERCISE</span><h2>Predict, run, explain</h2><p>${esc(guide
